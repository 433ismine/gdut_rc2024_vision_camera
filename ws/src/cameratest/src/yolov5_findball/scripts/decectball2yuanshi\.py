...
class Yolo_Dect:
        ...
        for box in boxs:
            if(box[-1]!='purple'):
                ...
                if(box[2]<=imagewidthmin  or box[0]>=imagewidthmax or box[1]>=imageheightmax):
                    self.balls.append(self.calculate(box[0],box[1],width)) # [ball_point,r,xmin,ymin]
                else:
                    xc = (box[0]+box[2])/2
                    yc = (box[1]+box[3])/2
    ...
    def calculate(self,xmin,ymin,width):
        ...
        c1= 2045.1
        d1= -0.0029
        d=a1*math.exp(b1*ymin) + c1*math.exp(d1*ymin)
        ...
        r=solution[0]/2
        print(r)    
        ball_point.x=1.0*(95.0/r)*(xmin+r-(width/2))
