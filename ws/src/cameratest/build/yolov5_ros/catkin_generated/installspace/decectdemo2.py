...
class Yolo_Dect:
    ...
    def judge(self,silos_data):
        ...
        if l == 5:
            for i in range(l):  
                if(self.silos[i]!=silos_data[i]):
                    if(self.previous_data[i] ==silo_data3[i]): 
                       self.silos[i]=silo_data3[i]
                    else:    
                        self.previous_data[i] = silos_data[i]
                else:    
                    self.previous_data[i] = silos_data[i]
    ...
    def pubjudge(self,currect_data, n):
        
        l = 5
        silos_last = ["null","null","null","null","null"]
        if currect_data == self.previous_data:
            self.previous_data = currect_data
            return currect_data
        else: 
            for i in range(l):
            if currect_data[i]!=self.previous_data[i]:
                if i+1 <5 and currect_data[i]==currect_data[i+1]:
                    silos_last[i]=currect_data[i]
        self.previous_data=currect_data
        ...
        for i in range(l):
            if count.get(silos[i], 0) >= n:
               silos_pub.append(silos[i])  
