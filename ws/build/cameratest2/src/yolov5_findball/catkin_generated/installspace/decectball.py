...
class Yolo_Dect:
    ...
    def __init__(self):
        ...
        # image subscribe
        self.color_sub = rospy.Subscriber("/camera/image_raw2", Image, self.image_callback,
    ...
    def dectshow(self, org_img, boxs, height, width):
        ...
        for box in boxs:
            if box[-1]=="blue":
                if (box[0]+box[2])/2 < 320+110:
                    if (box[0]+box[2])/2 > 320-110:
                        self.balls.append(box)
                elif (box[0]+box[2])/2 < 220:
                    if (box[0]+box[2])/2 > 0:
                        self.balls.append(box)
                elif (box[0]+box[2])/2 < 640:
                    if (box[0]+box[2])/2 > 640 -220:
                        self.balls.append(box)
